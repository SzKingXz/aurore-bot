...
async function handleSelectMenu(interaction, client) {
  if (interaction.customId === 'help_category') {
    const HELP_DATA = require('../../utils/constants').HELP_DATA ?? {};
    const data = HELP_DATA[interaction.values[0]];
    if (!data) return interaction.reply({ content: '❌ Categoría no encontrada.', ephemeral: true });
  ...
  if (interaction.customId === 'select_color') {
    ...
    if (!role) return interaction.reply({ content: '❌ Color no encontrado.', ephemeral: true });

    const existing = interaction.member.roles.cache.filter(r => COLOR_ROLES.some(cr => cr.name === r.name));
    for (const [, r] of existing) await interaction.member.roles.remove(r).catch(() => {});
    ...
    return interaction.reply({ content: `❌ El rol **${role.name}** no existe en este servidor.`, ephemeral: true });
  }
  ...
  if (interaction.customId === 'select_region') {
    ...
    if (!region) return interaction.reply({ content: '❌ Región no encontrada.', ephemeral: true });

    const existing = interaction.member.roles.cache.filter(r => REGION_ROLES.some(rr => rr.name === r.name));
    for (const [, r] of existing) await interaction.member.roles.remove(r).catch(() => {});
    ...
    return interaction.reply({ content: `❌ El rol **${region.name}** no existe en este servidor.`, ephemeral: true });
  }
}
